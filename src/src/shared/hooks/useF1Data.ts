...
/**
 * Busca a carreira INTEIRA do piloto (todas as temporadas) direto da API,
 * já somando temporada atual + passado — não precisa mais manter números
 * de carreira parados manualmente no JSON local.
 */
export function useDriverCareerStats(appDriverId: string) {
  ...
  return useQuery({
    ...
    staleTime: 24 * 60 * 60 * 1000, // carreira muda pouco fora de dia de corrida
