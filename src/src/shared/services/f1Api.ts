...
/**
 * Busca TODOS os resultados da carreira do piloto (todas as temporadas em
 * que ele correu) e calcula os totais a partir daí. Evita ter que "somar"
 * um número de carreira congelado com a temporada atual — o total sai
 * sempre correto, direto da fonte, sem risco de contar corrida duas vezes.
 */
export async function fetchDriverCareerStats(ergastDriverId: string): Promise<DriverCareerStats> {
  const pageSize = 1000;
  let offset = 0;
  let total = Infinity;
  // Guardamos junto a "chave" da corrida (temporada+rodada) pra poder
  // deduplicar depois — se a API repetir uma corrida entre páginas por
  // qualquer motivo, isso evita contar ela duas vezes.
  const raceResultMap = new Map<string, { season: string; result: ErgastResult }>();
  ...
  while (offset < total) {
    ...
    // A API pode devolver uma página menor do que o "limit" pedido — usamos
    // o limite REAL retornado para avançar, senão corremos o risco de pular
    // resultados (ex.: perder justamente as corridas mais recentes).
    ...
    for (const race of races) {
      ...
      for (const result of race.Results) {
        ...
        raceResultMap.set(key, { season: race.season, result }); // sobrescreve se repetido, nunca duplica
      }
    }
    if (races.length === 0) break; // segurança contra loop infinito
    offset += effectiveLimit;
  }

  const allEntries = Array.from(raceResultMap.values());
  ...
  for (const { season, result: r } of allEntries) {
    const pos = Number(r.position);
    // A API atual (jolpica) usa um vocabulário próprio de status, diferente
    // da Ergast antiga: "Finished" e "Lapped" são corridas COMPLETADAS
    // (o "+1 Lap" antigo virou a palavra "Lapped"). Mantemos o "+" como
    // segurança extra para dados antigos que ainda usem o formato velho.
    const finishedClassified =
      r.status === "Finished" || r.status === "Lapped" || (r.status?.startsWith("+") ?? false);

    // Só conta vitória/pódio se o resultado for realmente uma corrida
    // completada e classificada (evita contar quem foi desclassificado
    // depois, mesmo que o campo "position" ainda mostre a posição antiga).
    if (finishedClassified) {
      if (pos === 1) wins++;
      if (pos >= 1 && pos <= 3) {
        podiums++;
        if (season === CURRENT_SEASON) seasonPodiums++;
      }
    }
    if (r.grid === "1") poles++;
  ...
  return { starts: allEntries.length, wins, podiums, poles, dnfs, seasonPodiums };
}
